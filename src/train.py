...
def init_protonet(opt):
    ...
    model = ProtoNet(x_dim=opt.img_chanel).to(device)
    return model
...
def train(opt, tr_dataloader, model, optim, lr_scheduler, val_dataloader=None):
    ...
    for epoch in range(opt.epochs):
        ...
        for batch in tqdm(tr_iter):
            ...
            # print('\n')
            print('current train Loss: {}, Current Train Acc: {}'.format(round(float(loss),3),round(float(acc),3)))
            loss.backward()
            optim.step()
...
##################################################333
    ...
    res = train(opt=options, tr_dataloader=tr_dataloader, val_dataloader=val_dataloader, model=model, optim=optim, lr_scheduler=lr_scheduler)

    best_state, best_acc, train_loss, train_acc, val_loss, val_acc = res
    # print('t train loss: {}, val loss: {}, train acc {},val acc'.format(
